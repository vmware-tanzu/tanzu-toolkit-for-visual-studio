...
namespace Tanzu.Toolkit.Services.CloudFoundry
{
    ...
    {
        Task<DetailedResult> LoginWithCredentialsAsync(string username, SecureString password);
        ...
        Task<DetailedResult> DeployAppAsync(AppManifest appManifest, string defaultAppPath, CloudFoundryInstance targetCf, CloudFoundryOrganization targetOrg,
        ...
        Task<DetailedResult<string>> GetSSOPromptAsync(string cfApiAddress, bool skipSsl = false);

        Task<DetailedResult> LoginWithSSOPasscodeAsync(string cfApiAddress, string passcode);
